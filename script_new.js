...
function createPetals() {
    ...
    petal.style.animation = loatPetals s linear forwards;
        ...
        btn.addEventListener('click', () => {
            ...
            for (const [key, color] of Object.entries(moods)) {
                if (btn.textContent.includes(key.split(' ')[2])) { // Extract 'smile', 'angry', 'dumb'
                    document.body.style.background = linear-gradient(135deg, , #f5f0ff);
                    setTimeout(() => {
...
function setTimeBasedMessages() {
    ...
    if (hour >= 5 && hour < 12) {
        // Morning message
        showTemporaryMessage("Good morning, beautiful ");
    } else if (hour >= 18 && hour < 22) {
        // Evening message
        showTemporaryMessage("Hope your evening is as lovely as you are ");
    } else if (hour >= 22 || hour < 5) {
        // Night message
        showTemporaryMessage("You should sleep... I'll wait here ");
    }
}
...
function createNightSky() {
    ...
    for (let i = 0; i < 200; i++) {
        ...
        // Twinkle animation
        star.style.animation = 	winkle s infinite alternate;
...
function createShootingStar(parent) {
    ...
    shootingStar.style.animation = shoot s linear forwards;
    ...
    style.textContent = 
        @keyframes shoot {
            ...
            100% { 
                transform: translateX(100vw) translateY(%);
                opacity: 0;
            }
        }
    ;
